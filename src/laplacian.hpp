...
template <typename T, int P, int Q> __constant__ T phi0_const[Q * (P + 1)];

template <typename T, int Q> __constant__ T dphi1_const[Q * Q];
...
__launch_bounds__(Q *Q *Q) __global__
  ...
  if (identity != 1) {
    ...
    //     \phi0_(i0)(x_(j0)) \phi0_(i1)(x_(j1)) \phi0_(i2)(x_(j2)) =
    //     (d\phi1_(j0)/dx)(q0) \phi0_(i0)(x_(j0)) \phi0_(i1)(x_(j1))
    //     \phi0_(i2)(x_(j2))
    //
    // We want to compute
    //
...
const std::map<int, int> q_map_gll = {{1, 1},  {2, 3},   {3, 4},  {4, 6},
                                      {5, 8},  {6, 10},  {7, 12}, {8, 14},
                                      {9, 16}, {10, 18}, {11, 20}};

const std::map<int, int> q_map_gq = {{1, 2},  {2, 4},   {3, 6},  {4, 8},
                                     {5, 10}, {6, 12},  {7, 14}, {8, 16},
                                     {9, 18}, {10, 20}, {11, 22}};

template <typename T> class MatFreeLaplacian {
public:
  using value_type = T;
  ...
  MatFreeLaplacian(int degree, int qmode, std::span<const T> coefficients,
    ...
    if (op_nq == degree + 1) {
      ...
      case 7:
        ...
        break;
      case 8:
        copy_phi_tables<8, 9>(mat, table);
        break;
      case 9:
        copy_phi_tables<9, 10>(mat, table);
        break;
      case 10:
        copy_phi_tables<10, 11>(mat, table);
        break;
      default:
      ...
      case 7:
        ...
        break;
      case 8:
        copy_phi_tables<8, 10>(mat, table);
        break;
      case 9:
        copy_phi_tables<9, 11>(mat, table);
        break;
      case 10:
        copy_phi_tables<10, 12>(mat, table);
        break;
      default:
        throw std::runtime_error("Unsupported degree");
      }
  ...
  template <typename Vector> void operator()(Vector &in, Vector &out) {
    common::Timer top("Laplacian Operator:" + std::to_string(degree));
    ...
    if (op_nq == degree + 1) {
      if (degree == 1)
        ...
        impl_operator<8, 9>(in, out);
      else if (degree == 9)
        impl_operator<9, 10>(in, out);
      else if (degree == 10)
        impl_operator<10, 11>(in, out);
      else
      ...
      if (degree == 1)
        ...
        impl_operator<8, 10>(in, out);
      else if (degree == 9)
        impl_operator<9, 11>(in, out);
      else if (degree == 10)
        impl_operator<10, 12>(in, out);
      else
        throw std::runtime_error("Unsupported degree [operator]");
    } else {
      throw std::runtime_error("Unsupported nq");
